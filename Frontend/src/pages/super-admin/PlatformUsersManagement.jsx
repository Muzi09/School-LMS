...
export function PlatformUsersManagement() {
  ...
  const [selectedRole, setSelectedRole] = useState(0) // 0 = SUPER_ADMIN, 4 = SALES_PERSON
  const [showPassword, setShowPassword] = useState(false)
  const [serverError, setServerError] = useState(null)
  const [successMessage, setSuccessMessage] = useState(null)

  const { data: platformUsers, isLoading } = useQuery({
    queryKey: ["superAdminPlatformUsers"],
    queryFn: superAdminService.getPlatformUsers,
  })

  const createMutation = useMutation({
    mutationFn: ({ values, role }) => superAdminService.createPlatformUser(values, role),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["superAdminPlatformUsers"] })
      queryClient.invalidateQueries({ queryKey: ["superAdminStats"] })
      setSuccessMessage(
        `Successfully created ${selectedRole === 0 ? "Super Admin" : "Sales Person"} account for ${data.first_name} ${data.last_name}!`
      )
      formik.resetForm()
      setTimeout(() => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-card p-5 sm:p-6 rounded-2xl border border-border shadow-xs">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground mt-1 leading-normal">
            Manage Super Admin platform administrators and Sales Person representatives.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 px-4">
            ...
            <p className="text-xs text-muted-foreground mt-1 max-w-sm mx-auto">
              Create an additional Super Admin or Sales Person account to collaborate on the platform.
            </p>
            <Button
              type="button"
              onClick={() => setIsModalOpen(true)}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Register a new Super Admin or Sales Person platform account
                  </p>
                </div>
              </div>
              <button
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <div>
                      <div className="font-bold text-xs text-foreground">Super Admin</div>
                  ...
                  >
                    ...
                    <span>
                      {createMutation.isPending
                        ? "Creating..."
                        : `Create ${selectedRole === 0 ? "Super Admin" : "Sales Person"}`}
                    </span>
                  </Button>
                </div>
              </form>
